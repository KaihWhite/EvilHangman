...
int main() {
    ...
    for (const auto & iter : game.dict){
        if(lengths.count(iter.size()) < 1){
            lengths.insert(iter.size());
        }
    }
    
    // Keep playing the game until the user decides otherwise
    while (true) {
        print("Please Enter the desired word length.\n");
        int word_length = get_integer("Length: ");


        while (lengths.count(word_length) < 1){
            print("Whoops, that was an invalid length. Please try again.");
            word_length = get_integer("Length: ");
        }

        int num_guesses = get_integer("How many guesses would you like?");
        cout << endl;

        bool wordsleft = get_yesno("Would you like to see how many words are left as possible choices? yes or no: ");
        ...
        while (!game.is_won() && !game.is_lost()) {
            ...
            if (already_guessed.size() == 0) {
                if(wordsleft) print("There are " << game.possiblewords.size() << " possible words left.");
